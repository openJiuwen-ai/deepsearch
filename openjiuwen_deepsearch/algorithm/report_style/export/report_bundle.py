...
def _decode_base64_payload(payload: str, field_name: str) -> bytes:
    ...
    Raises:
        ReportStyleValidationError: base64 内容非法时抛出。
    """
    try:
        return base64.b64decode(payload, validate=True)
    except ValueError as exc:
        raise ReportStyleValidationError(f"invalid base64 payload for {field_name}") from exc
...
def _validate_bundle_id(raw_value: object, field_name: str) -> str:
    ...
    Raises:
        ReportStyleValidationError: ID 包含路径分隔符或其他不安全字符时抛出。
    """
    value = "" if raw_value is None else str(raw_value).strip()
    if not value:
        return ""
    if not SAFE_BUNDLE_ID_RE.fullmatch(value):
        raise ReportStyleValidationError(
...
def _validate_message_list(final_result: dict, field_name: str) -> list[dict]:
    ...
    Raises:
        ReportStyleValidationError: 字段不是列表或列表成员不是字典时抛出。
    """
    messages = final_result.get(field_name, [])
    if messages is None:
        return []
    if not isinstance(messages, list):
        raise ReportStyleValidationError(f"{field_name} must be a list")
    for index, item in enumerate(messages):
        if not isinstance(item, dict):
            raise ReportStyleValidationError(f"{field_name}[{index}] must be a dictionary")
    return messages
...
def _validate_final_result(final_result: dict) -> tuple[str, list[dict], list[dict]]:
    ...
    Raises:
        ReportStyleValidationError: final_result 结构不合法时抛出。
    """
    if not isinstance(final_result, dict):
        raise ReportStyleValidationError("final_result must be a dictionary")

    response_content = final_result.get("response_content", "") or ""
    if not isinstance(response_content, str):
        raise ReportStyleValidationError("response_content must be a string")
    if not response_content:
        raise ReportStyleValidationError("response_content is empty")
...
def build_report_bundle(final_result: dict, workspace: Path) -> ReportBundle:
    ...
    Raises:
        ReportStyleValidationError: `response_content` 为空或资源内容非法时抛出。
    """
