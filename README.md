...
**_1. 接入的系列模型，需要支持关闭思考模式能力，或者具备非思考模式能力。例如，qwen3-max、qwen-plus-latest等。_**

**_2. 除HITL/终止等针对同一个任务的场景外，每次调用deepsearch SDK的run接口使用服务时，需要传入不同的conversation_id，不允许使用相同的conversation_id二次传入。_**
