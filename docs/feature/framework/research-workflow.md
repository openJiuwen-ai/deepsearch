...
6. HITL 开启时进入 `GenerateQuestionsNode` 和 `FeedbackHandlerNode`；否则直接进入 `OutlineNode`。
7. `OutlineNode` 按 `search_context.outline_execution_method` 选择普通大纲 prompt/tool schema 或依赖驱动大纲 prompt/tool schema，并在必要时经 `OutlineInteractionNode` 进行多轮修订。
