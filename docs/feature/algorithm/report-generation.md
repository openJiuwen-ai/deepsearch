...
本文档覆盖 `openjiuwen_deepsearch/algorithm/report/` 下的报告生成能力，包括子报告生成、资料分类、候选文档预筛、Markdown 标题清理、表格标题归一、可视化 Mermaid 片段生成和最终报告拼接。
...
3. LLM 对资料进行分类，生成章节可用的 classified contents。
4. 子报告 Prompt 根据章节契约、资料和历史上下文生成 Markdown。
