...
- 意图识别从 `original_query` 提取两个可空时间约束：`source_date_scope`（来源发表/可得时间，硬门）与 `content_date_scope`（事实/事件/研究/数据时段，软分），各含可选 `start_date`/`end_date`，两者可并存，无需用户显式开关。旧单值 `temporal_scope` 字段保留但已 deprecated，仅供旧 state 输入路由。
...
- 意图识别：`openjiuwen_deepsearch/algorithm/query_understanding/intent_recognition.py`；Prompt `algorithm/prompts/intent_recognition/`
...
- `ResearchIntent.source_date_scope` / `content_date_scope`：两个可空 `TemporalScope`，分路硬门/软分；旧 `temporal_scope` 字段 deprecated，仅供输入路由，构造后恒为 None。
- `TemporalScope`：`constraint_type`（必填，须与所在字段名一致，不一致置 None + warning）、`start_date`/`end_date`（可选 `date`，ISO 闭区间）。
...
- 意图识别 LLM 无 tool_call 或异常时回退为无时间约束（两个 scope 均为 None，不阻断流程）。
