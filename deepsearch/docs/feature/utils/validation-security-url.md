...
6. 本地调试或内网部署可通过显式环境变量放宽 runtime API、embedding 或搜索服务 URL 校验（`RUNTIME_API_ALLOW_UNSAFE_URL` / `EMBEDDING_SERVICE_ALLOW_UNSAFE_URL` / `SEARCH_SERVICE_ALLOW_UNSAFE_URL`）。
