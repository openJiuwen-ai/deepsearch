...
- `exclusion_constraint_enable` 从请求透传到 Agent 配置，用于控制是否启用子报告禁引约束（采集层文献 ID 匹配 + 镜像后缀词表、写作层 Excluded Sources 注入、intent 层 include_url/target_papers 去重）。默认关闭。
...
- `DeepSearchRequest.exclusion_constraint_enable` 默认 `False`，透传为 `AgentConfig.exclusion_constraint_enable`。
