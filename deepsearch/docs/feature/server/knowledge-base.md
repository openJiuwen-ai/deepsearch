...
- 搜索和列表接口仅返回知识库元数据、状态和图增强信息；不会返回模型凭据或完整 config。
- 删除知识库会删除数据库记录、本地目录，并尽力删除 Milvus 中的 chunks/triples 索引。
- 上传时 `metadata.doc_list` 中的文档 ID 必须是非路径标识符；包含绝对路径、目录遍历或路径分隔符的请求会返回 400，文件始终保留在对应知识库的存储目录内。
...
- 上传请求包含不安全的文档 ID 时返回 400，不会创建文件或执行文档同步删除。
- 当前知识库模块缺少集中测试文件，修改时应优先补充 server 侧测试。
- 持久化的模型配置仅供 server 内部运行时读取，不能直接序列化到知识库搜索或列表响应。
