...
- 大纲交互（revise_outline/revise_comment）在升级运行中正常进行；交互修订
  轮次仍受 brief 大纲标题一致性约束。
...
- `openjiuwen_deepsearch/framework/openjiuwen/agent/main_graph_nodes.py`：
  ...
  `BriefOutline.matches_section_titles` 做标题一致性校验（领域契约在
  algorithm 层）。
...
- `openjiuwen_deepsearch/algorithm/prompts/outliner.md`：注入场景条件块。
...
6. 生成结果做标题序列一致性校验（归一化编号噪声后比较），不一致则按既有重试
   机制重试，重试耗尽走既有 exception_info 错误路径。
7. 校验通过后进入大纲交互（如启用），后续流程与普通专业版一致。
...
- 仅在升级运行首轮生效：注入发生在 StartNode，后续轮次（大纲交互反馈等）由
  workflow 状态机自然接管；`brief_state` 在运行内保留，使修订轮次仍受标题
  一致性约束。
...
- 标题一致性校验重试耗尽走既有 `OUTLINER_GENERATE_ERROR` 路径，不新增错误码。
...
（StartNode）、注入场景章节数与标题校验重试（OutlineNode）、/run 校验 400 与
