...
- `build_exclusion_prompt_context(intent)` 为写作 Prompt 生成禁引约束上下文：读 `research_intent.exclude_url` / `exclude_titles`，生成两段指令——禁止引用/提及/转述被禁源（即使已混入材料池），以及 silent constraint（不在报告里复述规则本身）。清单各取 top-20 防止动态 user prompt 膨胀，超出部分靠采集层确定性过滤兜底。**受 `exclusion_constraint_enable` 总开关控制**：关闭时不注入（baseline 行为，writer 对禁引清单一无所知）；开启时注入。两个注入入口：Professional 在 `report.py` 的 SubReporter（模板 `sub_report_markdown/`），Brief 在 `BriefSubReporterNode`（模板 `brief_sub_reporter/`），均与 `build_temporal_scope_prompt_context` 并列。
