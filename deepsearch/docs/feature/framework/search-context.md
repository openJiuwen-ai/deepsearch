...
- `build_exclusion_prompt_context(intent)` 为 `sub_report_markdown` 写作 Prompt 生成禁引约束上下文：读 `research_intent.exclude_url` / `exclude_titles`，生成两段指令——禁止引用/提及/转述被禁源（即使已混入材料池），以及 silent constraint（不在报告里复述规则本身）。清单各取 top-20 防止 system prompt 膨胀，超出部分靠采集层确定性过滤兜底。**受 `exclusion_constraint_enable` 总开关控制**：关闭时不注入（baseline 行为，writer 对禁引清单一无所知）；开启时注入。注入入口在 `report.py` 的 SubReporter，与 `build_temporal_scope_prompt_context` 并列。
