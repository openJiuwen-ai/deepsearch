...
7. 标题编号和过深标题被清理，并校验 Markdown 标题与子大纲逐项一致；章节草稿同时校验不含 Mermaid/图表源码；任一章节校验失败，生成受控重试反馈并重新生成章节。
8. 未启用受控可视化插入时，汇总报告还会再次拒绝 Mermaid/图表源码，防止已有正文或其他路径绕过章节重试；拒绝时不对正文做本地删除。
