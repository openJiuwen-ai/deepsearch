...
- professional 和 brief 写作 Prompt 都无条件禁止正文模型直接输出 Mermaid 代码围栏、图表代码或手写图块；即使 Mermaid 可视化管线关闭、改由 VLM 图表模块生成图片，章节正文也只能输出可溯源的文字和表格，受控图表插入由后续图表管线处理。VLM 模式下若模型仍返回 Mermaid/图表源码，章节输出契约会拒绝该草稿并通过有界重试要求模型改写，不对正文做事后删除。
...
7. 标题编号和过深标题被清理，并校验 Markdown 标题与子大纲逐项一致；VLM 模式还会校验章节草稿不含 Mermaid/图表源码；任一章节校验失败，生成受控重试反馈并重新生成章节。
8. 汇总报告在交给 VLM 图表节点前再次拒绝 Mermaid/图表源码，防止已有正文或其他路径绕过章节重试；拒绝时不对正文做本地删除。
9. 生成或更新 chapter sidecar。
10. 子报告交给最终报告拼接。
