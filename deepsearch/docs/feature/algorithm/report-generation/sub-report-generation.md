...
- 所有传入 LLM prompt 的 outline 均经 `export_outline_without_plans` 处理：剥离 `plans`（含 `step_result`/`evaluation` 等收集结果全文）和仅供图表规划使用的 `visualization_requirements`，仅保留章节标题、描述、依赖关系等写作结构骨架，避免超长输入或图表指令污染正文。
...
- VLM 图表模式下，正文还必须自洽，不写图题或“下图/见图/如下图”等依赖后续图片的前向引用；图表选择、图片生成、图题和插入由 VLM 管线独立负责，系统不会在 VLM 节点之后扫描或删除正文。
