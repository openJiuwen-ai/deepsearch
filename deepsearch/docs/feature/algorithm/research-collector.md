...
- 归一化时按需附加 `date_metadata`：引擎已归一化的发表日期为 day 粒度/high 置信，缺失时以 URL 路径日期兜底（medium 置信，year/month/day 粒度）。
- `temporal_scope.constraint_type == "source_date"` 时，tavily/google/common 三条路径统一执行后置时间过滤：日期与约束归一化为闭区间判定，只硬删高置信越界记录；日期未知和中低置信越界一律保留，交给下游软着陆排序。
