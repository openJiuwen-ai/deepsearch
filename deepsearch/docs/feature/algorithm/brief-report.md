...
报告类型有三种来源：`/run` 请求的 `report_type` 入参（或 `/run_brief` 薄封装，强制 `brief`）、用户澄清反馈中明确指定的“精简版”，以及意图识别从原始问题中的判定。API 指定值优先级最高：意图识别不再判定类型，反馈不可覆盖，且仅首轮生效、会话内不可变；未指定时保持原有判定与澄清机制。
...
- 正文写作不输出 Mermaid；Brief 不再有任何 Mermaid 流程，`visualization_enable` 配置对 brief 不再生效（仅 professional 生效）。Brief 不进入 `VLMChartGeneratorNode`。
...
`add_source_to_report` 引用整理），不调用 LLM、不生成图表；`visualization_enable` 配置对 brief
不再生效（仅 professional 生效）。
...
与图片引用统一清洗为 `[n]`；LLM 按零 script 契约输出（图表占位元素 + `template#chart-configs`
配置 JSON）；Python sanitizer（allowlist，清理后重新序列化）→ 校验（结构/零 script/CSS 外链/
图表配置与 option 递归安全/章节完整性/引用完整性）→ 失败带具体错误重试（复用
`report_max_generate_retry_num`，截断附加精简指令）→ 重试耗尽节点失败（`REPORT_GENERATE_ERROR`），
不提供降级产物；通过后确定性生成图表初始化脚本并内嵌 `echarts.min.js`（版本固定 + SHA-256 校验）、
插入 AI 声明。产物契约：`final_result.response_content = HTML`、
...
- HTML 生成复用 `report_max_generate_retry_num` 重试；重试耗尽时节点失败（`REPORT_GENERATE_ERROR`），不提供降级产物。
