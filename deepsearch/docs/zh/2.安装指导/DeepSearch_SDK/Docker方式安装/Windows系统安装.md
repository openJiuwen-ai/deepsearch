...
* 运行以下命令下载版本包：
  ...
  ```
  # 下载 x86_64 架构版本包：
  docker pull swr.cn-north-4.myhuaweicloud.com/openjiuwen/deepsearch-studio-server-amd64:0.1.10
  
  # 下载 ARM64 架构版本包：
  docker pull swr.cn-north-4.myhuaweicloud.com/openjiuwen/deepsearch-studio-server-arm64:0.1.10
  ```
...
* 推荐启动命令如下（SQLite；含 8089 映射，适用于 **DeepSearch** 模式 `search_mode=search`）：
  ...
  docker run \
    ...
    -e DB_TYPE=sqlite \ 
    swr.cn-north-4.myhuaweicloud.com/openjiuwen/deepsearch-studio-server-amd64:0.1.10
  ```
...
本地知识库依赖向量存储时，需设置 `INDEX_MANAGER_TYPE=milvus` 并配置 Milvus 连接。**DeepSearch 服务镜像内不包含 Milvus**，需要在宿主机或其他容器中**单独部署并启动** Milvus（例如 [Milvus 单机 Docker 说明](https://milvus.io/docs/install_standalone-docker.md)、自写 docker-compose，或 Agent Studio 等环境中已提供的 Milvus 容器）。
  ...
  **网络说明**：
  ...
  - **Milvus 在 WSL2 / 宿主机本机进程监听 `19530`**：DeepSearch 若在 Docker Desktop 的 Linux 容器内，可设 `MILVUS_HOST=host.docker.internal`、`MILVUS_PORT=19530`（或实际监听端口）。
...
#### Checkpointer 配置
  ...
  **镜像说明**：
  ...
  - **官方镜像**：从华为云 SWR 拉取，启动命令最后一行使用已 `docker pull` 的完整镜像名，例如  
    `swr.cn-north-4.myhuaweicloud.com/openjiuwen/deepsearch-studio-server-arm64:0.1.10`（ARM64）或  
    `swr.cn-north-4.myhuaweicloud.com/openjiuwen/deepsearch-studio-server-amd64:0.1.10`（x86_64）。拉取失败多为网络或权限问题，可检查代理、镜像加速或改用本地构建。
