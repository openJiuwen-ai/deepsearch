...
工作流结束时，最终结果统一写入 `final_result`（见 [search_context 文档](../3.开发指南/API文档/search_context.md)）。排查时优先看 **`exception_info`** 与 **`warning_info`**，再结合日志中的 **`conversation_id` / `thread_id`** 串联全链路。
