...
When a workflow ends, the final output is written to `final_result` (see the [search_context reference](../3.Developer%20Guide/API%20Reference/search_context.md)). Start with **`exception_info`** and **`warning_info`**, then use **`conversation_id` / `thread_id`** in the logs to follow the full request.
