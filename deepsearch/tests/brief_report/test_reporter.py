...
async def test_summary_includes_coverage_gaps_without_preemptive_trimming(monkeypatch):
    ...
    def prompt(_name, data):
        ...
        return [{"role": "system", "content": "固定模板"}, {"role": "user", "content": data["main_content"]}]
    invoke=AsyncMock(return_value={"content":"<executive_summary>- 核心。[citation:1]</executive_summary>"})
    monkeypatch.setattr("openjiuwen_deepsearch.algorithm.brief_report.writer.build_prompt_messages", prompt)
    ...
    assert "证据。" * 300 in captured["main_content"]
    assert "Known evidence limitations:" in captured["main_content"]
    assert "缺少数据" in captured["main_content"]
...
async def test_summary_receives_user_format_and_citation_bearing_excerpt_after_runtime_limit(monkeypatch):
    ...
    assert "## Heading" in captured["main_content"]
    assert "Fact retained.[citation:1]" in captured["main_content"]
