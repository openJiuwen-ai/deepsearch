...
class ExposedFigurePlaceholderGenerator(FigurePlaceholderGenerator):
    """用于测试的类，公开受保护的方法以遵循 G.CLS.11 规则"""

    async def process_section_h2(self, *args, **kwargs):
        return await super()._process_section_h2(*args, **kwargs)

    async def process_section_h1(self, *args, **kwargs):
        return await super()._process_section_h1(*args, **kwargs)


@pytest.mark.asyncio
async def test_process_section_h2_returns_empty_list_on_llm_error():
    """A failed LLM call yields an empty list, never None."""
    generator = ExposedFigurePlaceholderGenerator("dummy-model")
    ...
    with patch(
        ...
        result = await generator.process_section_h2(section)
...
async def test_process_section_h1_skips_none_and_exception_results():
    ...
    with patch.object(
        ...
        result = await generator.process_section_h1(sections)
