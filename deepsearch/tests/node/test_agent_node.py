...
def test_vlm_post_handle_preserves_mermaid_content_from_chart_generator():
    """VLM post handling must not rewrite chart-generator output."""
    ...
    assert current_report.report_content == report_content
...
def test_vlm_post_handle_does_not_sanitize_report_when_generation_fails():
    """A failed VLM run must not mutate the report body as a side effect."""
    node = VLMChartGeneratorNode()
    current_report = Mock(report_content="", merged_trace_source_datas=[])
    session = Mock(spec=Session)
    session.get_global_state.side_effect = (
    ...
    assert current_report.merged_trace_source_datas == []
