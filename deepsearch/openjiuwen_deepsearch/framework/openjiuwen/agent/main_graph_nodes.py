...
class OutlineNode(BaseNode):
    ...
    def _select_prompt_and_dep_driving(self, current_inputs: dict) -> tuple[str, bool, str]:
        ...
        if current_inputs.get("brief_outline"):
            # 注入场景：结构约束条件块仅在 outliner prompt 中，统一走普通大纲工具（PARALLEL）
        ...
        while not success_flag:
            ...
            if success_flag and current_inputs.get("brief_outline"):
                ...
                if not parsed_brief.matches_section_titles([s.title for s in outline_candidate.sections]):
                    logger.warning(
                        f"{self.log_prefix} Injected brief outline title mismatch, retrying."
                    )
                    algorithm_output["success_flag"] = False
                    algorithm_output["error_msg"] = "injected brief outline section titles mismatch"
                    success_flag = False
