...
* **Execution**:
  1. Synthesize the core status/conclusion from the previous summary (Context: the previous title).
  2. Naturally flow into the main driver/theme of the next summary (Context: the next title).
...
* **Execution**: Summarize the core essence of the next summary using the next title as the thematic anchor.

### Scenario C: Only Previous Summary Exists (Conclusion)
* **Goal**: Treat this as a closing summary.
* **Execution**: Conclusively wrap up the key insights of the previous summary within the scope of the previous title
