...
You will be given content from ONE user-provided material (paper, article, table, or dataset note), identified by the material title in the current user message, and the user's original research query. Summarize it for downstream research planning and report writing.
...
6. Output ONLY the summary text (no headings like "Summary:", no meta commentary). Keep it under the token limit supplied in the current user message.
