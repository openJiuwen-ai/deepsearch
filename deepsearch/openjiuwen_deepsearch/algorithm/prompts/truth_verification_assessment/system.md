...
- Markdown shown to end users as-is; write entirely in the requested language
- First line: a bold conclusion label in the requested language + a natural-language judgment that matches `conclusion`; do not put enum codes such as `supported` or `unsupported` in `display_text`
...
- Optional references section in the requested language: for Chinese use `**参考来源**`, otherwise use `**References**`; list `- [title](url)`, at most 10 links
