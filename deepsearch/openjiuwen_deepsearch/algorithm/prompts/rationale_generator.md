...
### Primary Rationales (from user-specified dimensions)

Identify explicit dimensions in the chapter description and user query. These become **primary rationales**.
...
4. **Preserve user intent**: Use the user's own terms — do not rename, merge, or substitute user-specified dimensions. If the user specified 5 dimensions, generate 5 primary rationales.
...
2. Cover analytical dimensions a high-quality report would address but the user did not explicitly name (e.g., if comparison is requested, cover "key differences and causes").
3. Primary rationales should account for at least 80% of total rationales when the user specified explicit dimensions.

### Guidelines

1. **Query-first when detailed (length > 200 characters).** Extract rationales primarily from the user query; skip gap-filling from step summaries unless it directly supports a query dimension.
2. Generate between 5 and 15 rationales. Lean toward upper bound when multiple entities/countries/periods are involved; lower bound for narrow topics.
...
5. Mark each rationale with `type`: "quantitative" (data/metrics), "qualitative" (analysis/opinion), "contextual" (background/definition), or "structural" (table/column/section format).
6. Mark each rationale with `priority`: "primary" (user-specified) or "supplementary" (gap-filling).
...
5. **Three-dimensional coverage** — rationales should collectively cover: information recall (facts/data), analysis (mechanisms/relationships), and presentation (required tables/columns/structure). Generate "structural" type rationales for format requirements.
...
User query (excerpt): *"评估育儿补贴政策对总和生育率的影响。对比中国、日本、韩国三国的补贴政策力度（金额、覆盖范围、发放周期）。用表格展示三国总和生育率变化（2015-2024年）。分析补贴政策对生育率的因果影响机制。"*
...
```json
{
  "rationales": [
    ...
    {"id": "r4", "description": "2015-2024年中日韩三国总和生育率数据表", "type": "structural", "priority": "primary"},
