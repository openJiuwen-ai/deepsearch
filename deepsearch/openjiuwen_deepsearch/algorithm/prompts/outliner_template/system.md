...
- The template refers only to the content inside <reference_report_template> in the current user message. The original request, research question, feedback, and entry search results are separate context, not part of the template. Instantiate generic terms such as "某城市", "某公司", or "该方法" with the specific entities and scope supplied by the user question or feedback, while preserving the template's structure.
- Do not leave generic placeholders or unresolved entity references in the generated outline when the user has supplied a concrete entity.
