...
def _escape_chart_config_template_payloads(html_text: str) -> str:
    ...
    Args:
        html_text: 可能包含 chart-configs template 的原始 HTML。

    Returns:
        将 template 内 JSON 作为文本节点转义后的 HTML。
    """
    parts: list[str] = []
    cursor = 0
    ...
    while match is not None:
        ...
        closing = None
        # 以括号平衡而非第一个 </template> 判定结束，避免 JSON 文本中的伪标签截断配置。
...
class _Sanitizer(HTMLParser):
    ...
    Attributes:
        out: 清理后重新序列化的 HTML 片段。
        _tag_stack: 当前已输出且等待闭合的标签栈。
        _chart_mode: 是否正在读取 chart-configs template 的 JSON 文本。
    """
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if self._chart_mode:
            if tag == "template":
                ...
                # 只有完整、平衡的 JSON 才回写 template；否则整块配置安全丢弃。
    ...
    def _render_attrs(self, tag: str, attrs: list) -> str:
        ...
        for name, value in attrs:
            ...
            text = value or ""
            if lowered == "href":
                # 链接只允许 http(s) 或页内锚点，避免把危险协议带入最终 HTML。
...
def sanitize_html(html_text: str) -> str:
    ...
    Args:
        html_text: LLM 生成的原始 HTML 文本。

    Returns:
        删除不允许标签、属性和不完整图表配置后的 HTML 文本。
    """
    parser = _Sanitizer()
...
class _EventAttributeScanner(HTMLParser):
    ...
    Attributes:
        has_event_attribute: 是否发现以 ``on`` 开头的元素属性。
    """
...
class _HtmlStructureScanner(HTMLParser):
    ...
    Attributes:
        chart_ids: 按文档顺序出现的图表占位 id。
        chart_configs_raw: chart-configs template 中的原始 JSON 文本。
        style_blocks: style 标签中的 CSS 文本。
        inline_styles: 元素 style 属性中的 CSS 文本。
    """
    ...
    def handle_endtag(self, tag: str) -> None:
        """结束对应的 style 或 chart-configs template 作用域。
    ...
    def handle_data(self, data: str) -> None:
        """收集当前 style 或 chart-configs template 中的文本。
...
def _decode_css_escape(match: re.Match) -> str:
    ...
    Args:
        match: 匹配 CSS 转义序列的正则对象。

    Returns:
        对应的 Unicode 字符；序列无效时返回空字符串。
    """
    try:
...
def _normalize_css(css: str) -> str:
    """删除无关 CSS 差异，生成便于安全检查的规范化文本。

    Args:
        css: 原始 CSS 文本。

    Returns:
        去除注释、解码转义并压缩空白后的大小写折叠文本。
    """
...
def _validate_css_references(scanner: _HtmlStructureScanner) -> list[str]:
    ...
    Args:
        scanner: 已扫描目标 HTML 的结构扫描器。

    Returns:
        首个 CSS 安全错误组成的列表；没有错误时返回空列表。
    """
...
def _insert_before(html_text: str, closing_tag: str, payload: str) -> str:
    ...
    Args:
        ...
        closing_tag: 作为插入锚点的结束标签。
        payload: 待插入文本。

    Returns:
        插入后的 HTML；找不到锚点时返回原文。
    """
...
def final_security_assert(html_text: str) -> None:
    ...
    Args:
        html_text: 即将交付的最终 HTML 文本。

    Raises:
        RuntimeError: 发现未被系统标记包裹的脚本、事件属性或危险标签。
    """
