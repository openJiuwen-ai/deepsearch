...
from server.deepsearch.core.manager.report_manager.conversion_utils import _is_likely_inline_math
        ...
        for match in LATEX_TOKEN_RE.finditer(text):
            if match.start() > cursor:
                hyperlink.append(_make_hyperlink_text_run(
                    text[cursor:match.start()], style_r_fonts, superscript))
            raw = match.group(0)
            latex = raw.strip("$").strip()
            if latex and _is_likely_inline_math(latex):
                try:
