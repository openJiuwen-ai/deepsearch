...
class TestSearchNodes:
    @staticmethod
    def test_free_text_name_case_insensitive():
    ...
    @staticmethod
    def test_combined_type_and_text():
        result = search_nodes(NODES, "{type:function} parse")
        assert [n["name"] for n in result.matches] == ["parse_file"]

    @staticmethod
    def test_edge_only_query_returns_empty():
    ...
    @staticmethod
    def test_limit_truncates_matches_not_total():
    ...
    @staticmethod
    def test_limit_negative_returns_all():
        result = search_nodes(NODES, "{type:function}", limit=-1)
        assert result.total == 2
        assert len(result.matches) == 2

    @staticmethod
    def test_sorts_by_name_within_type():
        result = search_nodes(NODES, "{type:class}")
        assert [n["name"] for n in result.matches] == ["BaseNode", "FileNode"]

    @staticmethod
    def test_tag_counts_and_combos():
        result = search_nodes(NODES, "{type:function}")
        assert result.tag_counts["type:function"] == 2
...
class TestSearchEdges:
    ...
    @staticmethod
    def test_sorts_by_confidence_desc():
        result = search_edges(EDGES, "{relation:CALLS}")
